...
export default function QuestionFormModal({
  ...
  // 1. Choice Options
  const [options, setOptions] = useState<Array<{ id: string; label: string; text_en: string; text_ta: string; is_correct: boolean; image_url?: string }>>([
    { id: 'opt_1', label: 'A', text_en: '', text_ta: '', is_correct: false, image_url: '' },
    { id: 'opt_2', label: 'B', text_en: '', text_ta: '', is_correct: false, image_url: '' },
  ]);
  // 2. True/False
  ...
  useEffect(() => {
    ...
    if (question) {
      ...
      if (question.topic_id) {
       ...
       if (question.images) {
         const firstImg = Array.isArray(question.images) ? question.images[0] : null;
         setQuestionImageUrl(firstImg ? firstImg.url : '');
       } else {
         setQuestionImageUrl('');
       }
  ...
  const handleAddOption = () => {
    ...
    setOptions([
      ...options,
      { id: `opt_${Date.now()}`, label: nextLabel, text_en: '', text_ta: '', is_correct: false, image_url: '' }
    ]);
  };
  ...
  const handleOptionImageChange = (index: number, val: string) => {
    setOptions(options.map((opt, i) => ({
      ...opt,
      image_url: i === index ? val : opt.image_url
    })));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: any = {
      ...
      images: questionImageUrl ? [{ url: questionImageUrl, caption: '' }] : undefined,
    };
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
              ...
              <div className="flex items-center space-x-3">
                <input
                  ...
                  placeholder="Paste Question Image URL here or use upload button"
                ...
                />
                <label className="flex items-center space-x-1.5 px-3.5 py-2 bg-accent/10 hover:bg-accent/20 text-accent rounded-xl text-xs font-bold border border-accent/20 cursor-pointer transition-all">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        handleImageUpload(file, (url) => setQuestionImageUrl(url));
                      }
                    }}
                  />
                </label>
              ...
              {questionImageUrl && (
                <div className="mt-3 relative inline-block">
                  <img
                    ...
                    className="h-24 w-auto rounded-lg border border-slate-200 object-contain bg-white"
                  />
                  <button
                    type="button"
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="flex items-center space-x-2">
                        <input
                          ...
                          className="flex-1 px-2.5 py-1 text-[11px] border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-accent font-medium text-slate-600"
                        />
                        <label className="flex items-center space-x-1 px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-[10px] font-bold border border-slate-300/60 cursor-pointer transition-all">
                          <Upload className="w-3.5 h-3.5" />
                          <span>Upload</span>
                          <input
                            type="file"
                            ...
                            onChange={(e) => {
                              const file = e.target.files?.[0];
                              if (file) {
                                handleImageUpload(file, (url) => handleOptionImageChange(index, url));
                              }
                            }}
                          />
                        </label>
                        {opt.image_url && (
                          <div className="relative">
                            <img src={opt.image_url} alt="" className="h-6 w-6 object-cover rounded border border-slate-200" />
                            <button
                              type="button"

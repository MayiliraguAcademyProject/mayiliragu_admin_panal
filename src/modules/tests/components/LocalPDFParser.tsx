          ...
          <div className="grid grid-cols-12 gap-4">
            ...
            {isSimulatorOpen && parsedQuestions[selectedQuestionIndex] && (
              <div className="col-span-12 lg:col-span-4 flex justify-center">
                <div className="w-[280px] h-[480px] bg-[#FAF9FF] rounded-[28px] overflow-hidden flex flex-col relative text-slate-800 shadow-xl border border-slate-200">
                  {/* Mock App Bar */}
                  <div className="bg-[#0F3CC9] pt-4 pb-2 px-3 text-white flex justify-between items-center text-[10px] font-bold">
                    <span>Q.{parsedQuestions[selectedQuestionIndex].number} of {parsedQuestions.length}</span>
                    <span className="truncate max-w-[120px]">{parsedQuestions[selectedQuestionIndex].questionEn.substring(0, 15)}...</span>
                  </div>

                  {/* Simulator Body */}
                  <div className="flex-1 overflow-y-auto p-3 space-y-3">
                    <div className="bg-white rounded-xl p-3 border border-slate-100 space-y-2 shadow-sm">
                      {parsedQuestions[selectedQuestionIndex].sharedContext && (
                        <div className="bg-slate-50 border rounded-lg p-2 text-[10px] text-slate-650 max-h-24 overflow-y-auto">
                          {parsedQuestions[selectedQuestionIndex].sharedContext}
                        </div>
                      )}
                      <p className="text-[11px] font-extrabold text-slate-900 leading-relaxed">
                        {parsedQuestions[selectedQuestionIndex].questionEn}
                      </p>
                      {parsedQuestions[selectedQuestionIndex].questionTa && (
                        <p className="text-[10px] font-bold text-slate-500 leading-relaxed font-sans mt-0.5">
                          {parsedQuestions[selectedQuestionIndex].questionTa}
                        </p>
                      )}
                          ...
                          return (
                            <div
                              ...
                              className={`p-2 rounded-lg border font-semibold transition-all ${
                                isCorrect ? 'bg-emerald-500/10 border-emerald-300 text-emerald-800' : 'bg-slate-50 border-slate-200 text-slate-700'
                              }`}
                            >
                              <span className="font-bold mr-1">{opt}.</span> {valEn}
                              {valTa && <div className="text-[9px] text-slate-500 font-bold font-sans mt-0.5">{valTa}</div>}
                            </div>
                          );
                        })}
                      </div>
                  ...
                  {/* Navigation Footer */}
                  <div className="bg-white border-t border-slate-100 p-2.5 flex justify-between items-center text-[10px] font-bold">
                    <button
                      onClick={() => setSelectedQuestionIndex(p => Math.max(0, p - 1))}
                      className="px-2.5 py-1 border border-slate-200 rounded-lg text-slate-500 hover:bg-slate-550"
                    >
                      Previous
                    </button>
                    <button
                      onClick={() => setSelectedQuestionIndex(p => Math.min(parsedQuestions.length - 1, p + 1))}
                      className="px-3.5 py-1 bg-[#0F3CC9] text-white rounded-lg hover:bg-blue-750"
                    >
                      Next
                    </button>
                  </div>
